...
# Description: This code uses data for discharge (Q) and concentration (DOC) from the Kuparuk in 2018.
...
# Read in data file
data_Q <- read.csv(file= "Kuparuk_Q_2018.csv", sep= ",",header= T)
...
Q.fun.2018 = approxfun(x=data_Q$min.since, y=data_Q$Discharge_m3s)
...
names(chem.2018)[names(chem.2018) == "DateTime"] <- "Date"

# Calculate Baseflow for chem.2018 using hydrostats package
baseflow <- baseflows(chem.2018, 0.975, n.reflected = 30, ts = "mean")

# Create variable for mean baseflow
mean.bf <- baseflow$mean.bf

# Calculate high flow for chem.2018 using hydrostats
highspells <- high.spells(chem.2018,quant=0.9,threshold=NULL,volume=TRUE)

# Create variable for high glow
highflow <- highspells$high.spell.threshold

# Sort chem.2018 from low to high Q, only selecting columns for date,Q, and DOC
chem.2018_sort <- chem.2018[order(chem.2018$Q),c(1,3,5)]

# Plotting
# Ploting Q over time
plot(chem.2018$Date,chem.2018$Q,ylab="Discharge (m3/s)",xlab="Date")

# Plotting DOC vs Q for sorted data
plot(chem.2018_sort$DOC_mgL,chem.2018_sort$Q,xlab="Discharge (m3/s)",ylab="DOC (mg/L)")

# Creating bins of discharge
Q_bin_1 <- subset(chem.2018_sort, Q > 0 & Q < mean.bf) # bin for baseflow
Q_bin_2 <- subset(chem.2018_sort, Q >= mean.bf & Q < highflow) # bin between baseflow and high flow
Q_bin_3 <- subset(chem.2018_sort, Q >= highflow) # bin for high flow

# Making plot window for 3 plots
par(mfrow=c(1,3), mar=c(4,4,1.5,1), cex=0.85)

# Plotting Q vs. DOC based on discharge bins
plot(Q_bin_1$Q,Q_bin_1$DOC_mgL,xlab="Discharge (m3/s)",ylab="DOC (mg/L)",pch=20,type="p",main="Baseflow",cex.main=1,log="xy",ylim=c(3,12))
plot(Q_bin_2$Q,Q_bin_2$DOC_mgL,xlab="Discharge (m3/s)",ylab="DOC (mg/L)",pch=20,type="p",main="Moderate Flow",cex.main=1,log="xy",ylim=c(3,12))
plot(Q_bin_3$Q,Q_bin_3$DOC_mgL,xlab="Discharge (m3/s)",ylab="DOC (mg/L)",pch=20,type="p",main="High Flow",cex.main=1,log="xy",ylim=c(3,12))
